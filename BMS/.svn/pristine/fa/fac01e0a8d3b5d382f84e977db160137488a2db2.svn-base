...
extern xSemaphoreHandle xSemaphoreReceivelwIP;
...
void ETH_IRQHandler(void)
{
	...
	if (ints & RXINTGROUP) {
		...
		xSemaphoreGiveFromISR( xSemaphoreReceivelwIP, &xHigherPriorityTaskWoken);
...
void Board_ENET_GetMacADDR(uint8_t *mcaddr)
{
	...
	if ((mac_address[0] == 0) && (mac_address[1] == 0)
			&& (mac_address[2] == 0) && (mac_address[3] == 0)
			&& (mac_address[4] == 0) && (mac_address[5] == 0))
		memcpy(mcaddr, boardmac, 6);
	else
		memcpy(mcaddr, mac_address, 6);
}
