...
char SendCmdBuffer[260];// __attribute__ ((section(".usb_buffer")));
...
/*		GLCDClearAppArea();
		...
		lcd_putstring("ERROR IN MSG FORMAT", LINE2);
		vTaskDelay(500);
		return 0;
	}
...
/*		GLCDClearAppArea();
		...
		lcd_putstring("MOB NO ERROR", LINE2);
		vTaskDelay(500);
		return 0;
	}
...
U8 GSMWaitForOk(char *Ptr,U8 TimeOut) {
	...
	if ((strstr(Ptr, ">") != '\0') || (strstr(Ptr,"OK") != '\0'))
		return 1;
	/*if (strchr(Ptr,'>')!='\0')
		return 2;*/
	return 0;
...
U8 DeleteAllMsg(){
	...
	memset(RecBuffer,0,sizeof(RecBuffer));
	memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
	GetGSMReciveData(RecBuffer,28,100);
	if (strstr(RecBuffer, ">") != '\0' || strstr(RecBuffer,"OK") != '\0')
				return 1;
	vTaskDelay(1000);
	return GSMWaitForOk(RecBuffer,100);
}
...
U8 GSMSendCmdForSelectingTextMode(U8 Index) {
	...
	vTaskDelay(1000);
	memset(RecBuffer,0,sizeof(RecBuffer));
	GetGSMReciveData(RecBuffer,16,100);
	if ((strstr(RecBuffer, ">") != '\0') || (strstr(RecBuffer,"OK") != '\0'))
			return 1;
	vTaskDelay(1000);
	return GSMWaitForOk(RecBuffer,100);
...
U8 GSMSendCmdForSelectingMobileNo() {
	...
	memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
	sprintf(SendCmdBuffer, "AT+CMGW=\"+919986571517\"\r\n");
	UART3_Send_Buffer((U8 *)SendCmdBuffer,strlen(SendCmdBuffer));
	memset(RecBuffer,0,sizeof(RecBuffer));
	vTaskDelay(1000);
	GetGSMReciveData(RecBuffer,18,100);
...
//	Buffer[0] = 0x1d;
//	Buffer[1] = 0x0d;
//	sprintf(SendCmdBuffer, "\r\n", Buffer);
//	UART3_Send_Buffer((U8 *)SendCmdBuffer,strlen(SendCmdBuffer));
	vTaskDelay(1000);
	memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
	GetGSMReciveData(RecBuffer,15,100);
...
void CheckForTextModeProtocol(){
	U8 check = 0, i = 0;
	memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
	sprintf(SendCmdBuffer, "AT+CSMP?\r\n");
	UART3_Send_Buffer((U8 *)SendCmdBuffer,strlen(SendCmdBuffer));
	memset(RecBuffer,0,sizeof(RecBuffer));
	vTaskDelay(1000);
	GetGSMReciveData(RecBuffer,22,10);
	if (strstr(RecBuffer, "+CSMP") != '\0')
	{
		if (strstr(RecBuffer, ",0,0") != '\0')
		{
			memset(RecBuffer,0,sizeof(RecBuffer));
			check = 1;
			if(check)
			...
			{
				memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
				sprintf(SendCmdBuffer, " AT+CSMP=49,167,0,0\r\n");
				UART3_Send_Buffer((U8 *)SendCmdBuffer,strlen(SendCmdBuffer));
				memset(SendCmdBuffer,0,sizeof(SendCmdBuffer));
				memset(RecBuffer,0,sizeof(RecBuffer));
				check = GSMWaitForOk(RecBuffer,100);
