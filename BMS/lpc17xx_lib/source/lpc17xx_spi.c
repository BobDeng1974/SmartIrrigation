 ...
 */
#define SSEL0_PORT		(0)
#define SSEL0_PIN	(1 << 16)

void Printer_spi_init(void)
{
	PINSEL_CFG_Type PinCfg;
	SPI_CFG_Type SPI_ConfigStruct;

	// SSEL0 P0.16 as GPIO, pull-up mounted but driven push/pull here
	...
	// SCK P0.15 alternate function 0b10
	...
	PinCfg.Pinnum    = 18;
	PINSEL_ConfigPin(&PinCfg);
	//GPIO_SetDir(SSEL0_PORT, SSEL0_PIN, 1);
	//GPIO_ClearValue(SSEL0_PORT, SSEL0_PIN);
...
void PrinterSendData(uint8_t *Buffer, int Length)
{
	int i;

	for (i = 0; i < Length; i++) {
		LPC_SPI->SPDR = (Buffer[i]) & SPI_SPDR_BITMASK;
...
void SPI_ConfigStructInit(SPI_CFG_Type *SPI_InitStruct)
{
	...
	SPI_InitStruct->ClockRate = 72000000;
	SPI_InitStruct->DataOrder = SPI_DATA_LSB_FIRST;
