...
static uint8_t  ADCIntilizeCount = 0;
...
** 
*****************************************************************************/
uint32_t ADCInit(uint8_t channelNum )
{
...
//  LPC_PINCON->PINSEL1 &= ~0x00300000;	/* P0.25 A0.3, function 01 */
  if(ADCIntilizeCount == 0)
  {
	  ADCPinselInitilization();
	  ADCIntilizeCount++;
  }
  ...
  LPC_ADC->ADCR = ( 0x01 << channelNum ) | 	/* SEL=1,select channel 0 on ADC0 */
		( ( pclk  / 1000000 - 1 ) << 8 ) |  /* CLKDIV = Fpclk / ADC_Clk - 1 */
...
void ADCPinselInitilization(){
	  LPC_PINCON->PINSEL1 |=  0x00010000;      // P0.24 A0.1
	  LPC_PINCON->PINSEL1 |=  0x00004000;		 // P0.23 A0.0
	  LPC_PINCON->PINSEL1 |=  0x00040000;       // P0.25 A0.2
	  LPC_PINCON->PINSEL3 |=  0xC0000000;      // P1.31 A0.5
	  LPC_PINCON->PINSEL0 |=  0x00000080;		 // P0.2 A0.6
	  LPC_PINCON->PINSEL0 |=  0x00000020;	 // P0.2 A0.7
}
