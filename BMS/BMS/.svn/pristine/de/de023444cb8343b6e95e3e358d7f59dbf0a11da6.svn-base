...
uint32_t UART0Status,UART3Status,UART1Status;
...
extern CIR_BUFFER UART0CirBuf,UART3CirBuf,UART1CirBuf;
    ...
    if ( LSRValue &LSR_RDR )    /* Receive Data Ready */
    {
      ...
      /* Note: read RBR will clear the interrupt */
        	 ch=LPC_UART0->RBR & UART_RBR_MASKBIT;
    		  //if(getCirBufAvailableSize(&UART0CirBuf)<1)
    			//  popFromCirBuf(&UART0CirBuf,&Dummy,1);

    		      		pushToCirBuf(&UART0CirBuf,&ch,1);
  ...
  if ( IIRValue == IIR_RLS )        /* Receive Line Status */
  {
    ...
    if ( LSRValue &LSR_RDR )    /* Receive Data Ready */
    {
      ...
      /* Note: read RBR will clear the interrupt */
    	 ...
    	 ch=LPC_UART3->RBR & UART_RBR_MASKBIT;
    		 // if(getCirBufAvailableSize(&UART3CirBuf)<1)
    			 // popFromCirBuf(&UART3CirBuf,&Dummy,1);

    		      		pushToCirBuf(&UART3CirBuf,&ch,1);
  ...
  if ( IIRValue == IIR_RLS )        /* Receive Line Status */
  {
    ...
    if ( LSRValue &LSR_RDR )    /* Receive Data Ready */
    {
      ...
      /* Note: read RBR will clear the interrupt */
    	 ...
    	 ch=LPC_UART1->RBR & UART_RBR_MASKBIT;
    		 // if(getCirBufAvailableSize(&UART3CirBuf)<1)
    			 // popFromCirBuf(&UART3CirBuf,&Dummy,1);

    		      		pushToCirBuf(&UART1CirBuf,&ch,1);
